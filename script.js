...
var currentPageValue = 1; //using global variable
...
function prevPage(){
  if(currentPageValue > 1){
    currentPageValue--;
    changePage(currentPageValue);
  }
}

function nextPage(){
  if(currentPageValue < numOfPages()){
    currentPageValue++;
    changePage(currentPageValue);
  }
}
...
function lastPage(){
  if(currentPageValue < numOfPages()){
    currentPageValue = numOfPages();
    changePage(currentPageValue);
  }
}

function numOfPages(){
  let countOfPages =Math.ceil(userDataJSON.length/recordsPerPage);
  return countOfPages;
}
...
function changePage(page){
  ...
  if(page < 1) currentPageValue = 1;
  if(page > numOfPages()) page = numOfPages();
  const startIndex = (page-1)*recordsPerPage;
  const endIndex =  startIndex + recordsPerPage;
  const currentPageData = userDataJSON.slice(startIndex,endIndex);
  //clear exist data
  const tboday = document.getElementById("table-body");
  tboday.innerHTML ="";
  //interate to current page data
  currentPageData.forEach(element => {
    const row = document.createElement("tr");
    row.innerHTML=`<td>${element.id}</td><td>${element.name}</td><td>${element.email}</td>`;
    tboday.appendChild(row);
  });
}
...
(function (){
  ...
  paginationDomContainer.appendChild(firstButton);
  ...
  paginationDomContainer.appendChild(prevButton);


  //buttons for number pages
  for(let i=1;i<= totalPages;i++){
    ...
    paginationDomContainer.appendChild(button);
  }
    ...
    paginationDomContainer.appendChild(nextButton);
  ...
  paginationDomContainer.appendChild(lastButton)
